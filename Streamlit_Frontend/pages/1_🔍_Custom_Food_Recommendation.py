...
class Display:
    ...
    '''def display_overview(self, recommendations):
        if recommendations is not None:
            ...
            with col2:
                selected_recipe_name = st.selectbox('Select a recipe', [recipe['Name'] for recipe in recommendations if 'Name' in recipe])
            st.markdown(f'<h5 style="text-align: center;font-family:sans-serif;">Nutritional Values:</h5>', unsafe_allow_html=True)
            
            # Ensure selected_recipe is defined before accessing its values
            ...
            if selected_recipe is not None:  # Check if a matching recipe is found
                options = {
...
'''
    def display_overview(self, recommendations):
        if recommendations is not None:
            ...
            col1, col2, col3 = st.columns(3)
            with col2:
                selected_recipe_name = st.selectbox('Select a recipe', [recipe.get('Name') for recipe in recommendations if isinstance(recipe, dict)])
            if selected_recipe_name:
                st.markdown(f'<h5 style="text-align: center;font-family:sans-serif;">Nutritional Values:</h5>', unsafe_allow_html=True)
                selected_recipe = next((recipe for recipe in recommendations if recipe.get('Name') == selected_recipe_name), None)
                if selected_recipe:  # Check if a matching recipe is found
                    options = {
                        "title": {"text": "Nutrition values", "subtext": f"{selected_recipe_name}", "left": "center"},
                        "tooltip": {"trigger": "item"},
                        "legend": {"orient": "vertical", "left": "left",},
