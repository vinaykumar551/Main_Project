...
class Display:
        ...
        with st.spinner('Generating recommendations...'): 
            ...
            for meal_name,column,recommendation in zip(meals,st.columns(len(meals)),recommendations):
                with column:
                    ...
                    st.markdown(f'##### {meal_name.upper()}')    
                    for recipe in recommendation:
                        if isinstance(recipe, dict):  # Add this check
                            recipe_name=recipe['Name']  # The error occurred here
                            expander = st.expander(recipe_name)
                            recipe_link=recipe['image_link']
                            recipe_img=f'<div><center><img src={recipe_link} alt={recipe_name}></center></div>'     
                            nutritions_df=pd.DataFrame({value:[recipe[value]] for value in nutritions_values})      
                            ...
                            print(f"Error: recipe is not a dictionary. Recipe: {recipe}")  # Print an error message
    ...
    '''def display_meal_choices(self,person,recommendations):    
        st.subheader('Choose your meal composition:')
    # Display meal compositions choices
        if all(isinstance(recommendation, list) for recommendation in recommendations) and all(isinstance(recipe, dict) for recommendation in recommendations for recipe in recommendation):
            breakfast_column,launch_column,dinner_column=st.columns(3)
            with breakfast_column:
                breakfast_choice=st.selectbox(f'Choose your breakfast:',[recipe['Name'] for recipe in recommendations[0]])
            with launch_column:
            ...
            print(f"Error: recommendations is not a list of dictionaries. recommendations: {recommendations}")  # Print an error message
...
if st.session_state.generated:
    ...
    # Initialize the person object with the appropriate values
            st.session_state.person = Person(age, height, weight, gender, activity, meals_calories_perc, weight_loss)

# Call display.display_recommendation
        display.display_recommendation(st.session_state.person, st.session_state.recommendations)
